...
- 이번에 진행한 프로젝트는 Color-Script 라는 visual studio extension 입니다.  
  개발을 하다보면 많은 변수를 사용하게 되는데 변수명을 통해 타입을 유추할 수도 있지만 좀 더 직관적으로 데이터 타입을 알 수 있다면  
  코드를 작성하는 사람과 읽는 사람 모두에게 편리함을 줄 수 있겠다는 생각으로부터 시작되었으며  
  저에게는 새로운 방향의 개발로서 도전하는 의미도 가진 프로젝트 입니다.
